...
# Нужны чтобы сделать классы для декораторов. Чтобы не повторять одни и те же декораторы в коде
# Чтобы разобраться - смотри видео. Тут очень громоздко и запутано


class ReadIntX:  # Дескриптор не данных (Без сеттера  и приоритет обычный)
    ...
    # def __set__(self, instanse, value):             # сеттер можно и добавить
    #     setattr(instanse, self.name, value)


class Integer:  # Дескриптор данных (С сеттером и геттером)
    @classmethod
    def verify_coord(cls, coord):
        if type(coord) != int:
            raise ValueError("Coordinates must be an integers")
...
# Все ниже это упрощенная альтернатива:
...
#
# class TableSheet:
#     def __init__(self, N: Cell, M: Cell):
#         self.cells = [[Cell() for n in range(int(N.value))] for n in range(int(M.value))]
#         c = 1.0
...
#
#
# table = TableSheet(Cell(3.0), Cell(5.0))
...
validate(self, string) - возвращает True, если string является строкой (тип str) и длина 
строки в пределах [min_length; max_length]. Иначе возвращается False.
...
#         return self.min_length <= len(string) <= self.max_length and type(string) == str
#
# class StringValue:
...
Класс PriceValue должен проверять, что присваивается вещественное или целочисленное значение в диапазоне [0; 10000], т.е. max_value = 10000. Если проверки не проходят, то соответствующие (прежние) значения меняться не должны.
...
#         self.max_length = max_length
#
#     def st_valid(self, val):
#         return type(val) == str and self.min_length <= len(val) <= self.max_length
#
#     def __set_name__(self, owner, name):
#         self.name = "_" + name
#
...
#
#     def num_valid(self, val):
#         return isinstance(val, (int, float)) and 0 <= val <= self.max_value
#
#     def __set_name__(self, owner, name):
#         self.name = "_" + name
#
...
где порядковый номер - номер телепередачи в сетке вещания (от 1 и далее); название - 
наименование телепередачи; длительность - время телепередачи (в секундах - целое число).
