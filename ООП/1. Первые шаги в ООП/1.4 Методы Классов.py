...
#     circle = 2
#
#     # def set_coords(self):                   # self создает ссылку на экземпляр класса
#     #     print("call set_coords method " + str(self))
...
# pt = Point()
# # pt.set_coords()
# # Point.set_coords(pt)                        # эквивалентно pt.set_coords()
# pt.set_coords(1, 2)
# pt2 = Point()
...
На экране должна появиться строка с соответствующим набором чисел, 
...
который бы на входе получал список FIELDS из названий локальных атрибутов (передается в атрибут fields) 
и список строк lst_in (передается в атрибут lst_values) и формировал бы в объекте класса 
StreamData локальные свойства с именами полей из fields и соответствующими значениями из lst_values.

Если создание локальных свойств проходит успешно, то метод create() возвращает True, 
иначе - False. Если число полей и число строк не совпадает, то метод create() возвращает False 
и локальные атрибуты создавать не нужно.
...
#         for i in lst:
#             self.lst_data.append(dict(zip(DataBase.FIELDS, i)))
#
#     def select(self, a, b):
...
#         lst = []
#         lst.append(eng)
#         if eng not in self.BUNCH.keys():
#             self.BUNCH.setdefault(eng, [rus])
#         else:
#             self.BUNCH.setdefault(eng, []).append(rus)
#
#     def remove(self, eng):
#         self.BUNCH.pop(eng)
#
#     def translate(self, eng):
#         return self.BUNCH.get(eng)
#
#
# tr = Translator()
# tr.add("tree", "дерево")
