...
sv = Server()
При этом, уникальный IP-адрес каждого сервера должен формироваться автоматически при 
создании нового экземпляра класса Server.
...
class Server:
    ...
    def __init__(self):
        ...
        self.router = None                  # делаем пустой метод для роутера чтобы был назначен позже из его класса

    def send_data(self, data):
        if self.router:                     # если сервер и роутер связаны
            self.router.buffer.append(data) # тогда пакет отсылается
...
class Router:
    ...
    def link(self, server):
        ...
        server.router = self                    # Вот тут создаем ссылку на экземпляр роутера в Server
    ...
    def send_data(self):
        for s in self.buffer:
            if s.ip in self.servers:
                self.servers[s.ip].buffer.append(s)
        self.buffer.clear()
